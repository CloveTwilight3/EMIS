// speech-recognition.ts - Enhanced speech recognition module for EMIS
...
interface GoogleCloudSpeechResult {
  transcript: string;
  confidence: number;
}
...
class SpeechRecognitionService {
  ...
  private async setupServiceClients(): Promise<void> {
  ...
  async recognizeSpeech(audioBuffer: Buffer, options: Partial<SpeechRecognitionOptions> = {}): Promise<RecognitionResult> {
    if (!this.initialized) {
      throw new Error('Speech recognition service not initialized');
    }

    const requestOptions: Required<SpeechRecognitionOptions> = {
      ...this.options,
      ...options
    };
  ...
  private async processWithGoogleCloudSpeech(audioFile: string): Promise<GoogleCloudSpeechResult | null> {
  ...
  private async processWithVAD(audioFile: string): Promise<VADResult> {
    if (!this.vadDetector) {
  ...
  setupIPCHandlers(): void {
    ipcMain.handle('recognize-speech', async (_event: IpcMainInvokeEvent, audioBuffer: Buffer): Promise<RecognitionResult> => {
      try {
